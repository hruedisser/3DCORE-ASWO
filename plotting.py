...
def generate_model(data_cache, t_launch):
    extracted_row = extract_row(data_cache.row)
    iparams = get_iparams_live(*extracted_row)
    model_obj = ToroidalModel(t_launch, **iparams)
    model_obj.generator()
...
def plot_insitu_results(
    ...
    if ensemble_data is not None:
        ...
        # plot mean flux rope
        #ax.plot(t_data, np.sqrt(bx**2 + by**2 + bz**2), color=c0, lw=lw_mean, linestyle="dashed")

        #ax.plot(t_data, bx, color=colors[0], alpha=1, lw=lw_mean, linestyle="dashed")
        #ax.plot(t_data, by, color=colors[1], alpha=1, lw=lw_mean, linestyle="dashed")
        #ax.plot(t_data, bz, color=colors[2], alpha=1, lw=lw_mean, linestyle="dashed")   

        # calculate and plot best (= min(RSME)) flux rope 
        
        model_obj = generate_model(data_cache, t_launch)
        outa = np.array(model_obj.simulator(t_data, data_cache.pos_data), dtype=object)
        outa = np.squeeze(outa[0])

        ax.plot(t_data, np.sqrt(np.sum(outa**2, axis=1)), alpha=0.5, linestyle='dashed', lw=lw_best, color=c0)#, label='parameters with min(eps)')
        ax.plot(t_data, outa[:, 0], alpha=0.5, linestyle='dashed', lw=lw_best, color=colors[0])
        ax.plot(t_data, outa[:, 1], alpha=0.5, linestyle='dashed', lw=lw_best, color=colors[1])
        ax.plot(t_data, outa[:, 2], alpha=0.5, linestyle='dashed', lw=lw_best, color=colors[2])
