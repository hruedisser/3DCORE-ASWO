...
class CustomObserver(object):
    ...
    def get(
        self,
        dtp: Union[str, datetime.datetime, Sequence[str], Sequence[datetime.datetime]],
        b_data: List[float],
        t_data: List[datetime.datetime],
        **kwargs: Any
    ) -> np.ndarray:
        ...
        if kwargs.pop("as_endpoints", False):
            ...
            if isinstance(dtp, str):
                ...
                dtp = [
                    datetime.datetime.fromisoformat(dt_str).replace(tzinfo=None)
                    for dt_str in dtp
                ]
            ...
            for dt in dtp:
                ...
                if isinstance(t_data[0], str):
                    ...
                    closest_index = min(
                        range(len(t_data)),
                        key=lambda i: abs(
                            datetime.datetime.fromisoformat(t_data[i]).replace(
                                tzinfo=None
                            )
                    ...
                    closest_index = min(
                        range(len(t_data)),
                        key=lambda i: abs(
                            t_data[i].replace(tzinfo=None) - dt.replace(tzinfo=None)
                        ),
                    )
        ...
        for dt in dtp:
            ...
            if isinstance(t_data[0], str):
                ...
                closest_index = min(
                    range(len(t_data)),
                    key=lambda i: abs(
                        datetime.datetime.fromisoformat(t_data[i]).replace(tzinfo=None)
                        - dt.replace(tzinfo=None)
                    ),
                )
