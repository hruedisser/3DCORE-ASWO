...
class ABC_SMC(BaseMethod):
    ...
    def run(
        ...
        **kwargs: Any
        ...
        # read kwargs
        balanced_iterations = kwargs.pop(
            "balanced_iterations", 3
        )  # number of iterations before eps gets set to eps_max to balance for multiple observersx
        data_kwargs = kwargs.pop(
            "data_kwargs", {}
        )  # kwargs to be used for the FittingData, usually not given
        eps_quantile = kwargs.pop(
            "eps_quantile", 0.25
        )  # which quantile to use for the new eps
        kernel_mode = kwargs.pop(
            "kernel_mode", "cm"
        )  # kernel mode for perturbing the iparams - covariance matrix
        output = kwargs.get(
            "output", None
        )  # If output is set, results are saved to a file
        ...
        summary_type = kwargs.pop(
            "summary_statistic", "norm_rmse"
        )  # summary statistic used to measure the error of a fit
        time_offsets = kwargs.pop(
            "time_offsets", [0]
        )  # value used to correct arrival times at observers

        workers = kwargs.pop(
            "workers", multiprocessing.cpu_count() - 1
        )  # number of workers
        jobs = kwargs.pop("jobs", workers)  # number of jobs
        use_multiprocessing = kwargs.pop(
            ...
            for iter_i in range(self.iter_i, iter_max):
                ...
                if iter_i >= iter_min:
                    if self.hist_eps[-1] < epsgoal:
                        logger.info(
                            "fitting terminated, epsgoal reached: eps < %s", epsgoal
                        )
                        kill_flag = True
                        break
...
def abc_smc_worker(*args: Any) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if iter_i == 0:
        ...
        model_obj = model_class(
            dt_0, **model_kwargs
        )  # model gets initialized with model kwargs

        model_obj.generator(
            random_seed=random_seed
        )  # generator is run to create the iparams array and the quaternions
    ...
    sort_index = np.argsort(
        [_.timestamp() for _ in data_obj.data_dt]
    )  # the index is sorted by time (sometimes this isn't the case by default)
