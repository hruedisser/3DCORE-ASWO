...
# === Load kernels_path from JSON config ===
def load_kernels_path(config_file=Path(__file__).resolve().parents[3] /'config.json'):
    with open(config_file, 'r') as f:
        config = json.load(f)
    return config['kernels_path']



# Load path once globally
kernels_path = load_kernels_path()
...
def GSE_to_HAE(df):
    df_HEE = GSE_to_HEE(df)
    df_HAE = HEE_to_HAE(df_HEE)
    return df_HAE


def HAE_to_GSE(df):
    df_HEE = HAE_to_HEE(df)
    df_GSE = HEE_to_GSE(df_HEE)
    return df_GSE
